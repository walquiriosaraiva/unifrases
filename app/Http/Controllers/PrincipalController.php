...
class PrincipalController extends Controller
{
    ...
    public function fetch(Request $request)
    {
        ...
        $tabela = DB::table('frase')
            ...
            ->whereRaw('LOWER(texto) LIKE ?', ['%' . strtolower($data['texto']) . '%'])
            ->where('status', '=', 'A')
            ->take(6)
            ->get();
    ...
    public function idiomapesquisado(Request $request)
    {
        $data = $request->all();
        $idiomas = DB::table('idioma')
            ->join('frase', 'idioma.id', '=', 'frase.idioma')
            ...
            ->groupBy('idioma.id', 'idioma.nome', 'idioma.imagem')
            ->orderBy('idioma.nome')
            ->get();
    ...
    public function idiomatraduzir(Request $request)
    {
        ...
        $frase = DB::table('frase')
            ->where('frase.status', '=', 'A')
            ->whereRaw('LOWER(frase.texto) LIKE ?', ['%' . strtolower($data['texto']) . '%'])
            ->first();
